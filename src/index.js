  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/" || url.pathname === "/health") {
      return json({
        ...
        mode: "read-only",
        credentialsConfigured: Boolean(env.T212_API_KEY && env.T212_API_SECRET),
      });
    }

    if (url.pathname === "/t212/test") {
      if (!env.T212_API_KEY || !env.T212_API_SECRET) {
        return json({ ok: false, error: "Trading 212 secrets are not configured." }, 503);
      }

      const auth = btoa(`${env.T212_API_KEY}:${env.T212_API_SECRET}`);
